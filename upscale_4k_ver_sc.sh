...
INPUT_FILE="input/sc_720p_1min.mp4"
...
if [[ ${NON_SR_FLAG} == '1' ]]; then
  ...
  time docker exec -it non_sr bash /app/upscale.sh -i ${INPUT_FILE} -filter_complex "scale=w=iw*3:h=ih*3" "${OUTPUT_DIR}/sc_720p_1min_4k.mp4" -y
fi
...
if [[ ${SR_FLAG} == '1' ]]; then
  ...
  time docker exec -it sr bash /app/upscale_with_sr.sh -i ${INPUT_FILE} -c:v mpsoc_vcu_h264 -c:a copy -filter_complex "scale_startrek=w=iw*3:h=ih*3:fpga=alveo:c=1" "${OUTPUT_DIR}/sc_720p_1min_4k_sr.mp4" -y
fi
...
if [[ ${STACK_FLAG} == '1' ]]; then
  ...
  time docker exec -it sr /app/stack.sh -hide_banner -i "${OUTPUT_DIR}/sc_720p_1min_4k.mp4" -i "${OUTPUT_DIR}/sc_720p_1min_4k_sr.mp4" -filter_complex hstack -y "${OUTPUT_DIR}/sc_720p_1min_4k_hstack.mp4"
fi
